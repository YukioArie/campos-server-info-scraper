...
async function initialize() {
  ...
  const caminhoArquivo = "lista_servidores_municipio_campos_v3.json";
  ...
  ///////////////////////////////////////////////////////////
  try {
    fs.writeFile("error_list_v3.json", jsonError, (err) => {
      if (err) {
        console.error("Erro ao escrever o arquivo JSON:", err);
        return;
      }
      console.log("Arquivo JSON salvo com sucesso:", "error_list_v3.json");
    });
  } catch (error) {
    console.error(error);
  }
  ...
  xlsx.writeFile(workbook, "lista_servidores_municipio_campos_v3.xlsx");
